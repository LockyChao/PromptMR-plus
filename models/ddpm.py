...
from . import model_utils, layers, normalization, utils
...
class DDPM(nn.Module):
    def __init__(self, config):
        ...
        self.np = np = config.model.np
        self.learnable_prompt = learnable_prompt = config.model.learnable_prompt
        ...
        for i_level in reversed(range(num_resolutions)):
            ...
            if all_resolutions[i_level] in attn_resolutions:
                modules.append(AttnBlock(channels=in_ch))
            if i_level != 0:
                modules.append(Prompt(np * i_level, 5, 16 * (5 - i_level), out_ch, learnable_prompt))
                modules.append(Upsample(channels=in_ch, prompt_channels = np * i_level, with_conv=resamp_with_conv))

        assert not hs_c
        modules.append(nn.GroupNorm(num_channels=in_ch, num_groups=16, eps=1e-6))
    ...
    def forward(self, x, labels, meta):
        ...
        #print(f"[DDPM] temb.shape = {temb.shape if temb is not None else 'None'}")
        ...
        # Downsampling block
        hs = [modules[m_idx](h)]
        #print(f"[DDPM] Step {m_idx}: h.shape = {hs[-1].shape}")
        m_idx += 1
        for i_level in range(self.num_resolutions):
            # Residual blocks for this resolution
            for i_block in range(self.num_res_blocks):
                h = modules[m_idx](hs[-1], temb)
                #print(f"[DDPM] Step {m_idx}: h.shape = {h.shape}")
                m_idx += 1
                if h.shape[-1] in self.attn_resolutions:
                    ...
                    m_idx += 1
                hs.append(h)
                #print(f"[DDPM] Step {m_idx}: hs appended with shape = {hs[-1].shape}")
