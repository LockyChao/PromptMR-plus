...
class AttnBlock(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.GroupNorm_0 = nn.GroupNorm(
            num_groups=16, num_channels=channels, eps=1e-6)
...
class Upsample(nn.Module):
    def __init__(self, channels, prompt_channels, with_conv=False):
        super().__init__()
        if with_conv:
            self.Conv_0 = ddpm_conv3x3(channels, channels)
        self.with_conv = with_conv
        self.attn = AttnBlock(channels+prompt_channels)
        self.reduce = nn.Conv2d(channels+prompt_channels, channels, kernel_size=1, bias=True)
...
class ResnetBlockDDPM(nn.Module):
    ...
    def __init__(self, act, in_ch, out_ch=None, temb_dim=None, conv_shortcut=False, dropout=0.1):
        ...
        if out_ch is None:
            out_ch = in_ch
        self.GroupNorm_0 = nn.GroupNorm(
            num_groups=16, num_channels=in_ch, eps=1e-6)
        self.act = act
        ...
        self.GroupNorm_1 = nn.GroupNorm(
            num_groups=16, num_channels=out_ch, eps=1e-6)
