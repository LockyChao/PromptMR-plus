...
class PromptMR(nn.Module):
    ...
    def forward(
        ...
        masked_kspace_tmean: torch.Tensor,
        mask: torch.Tensor,
        ...
        if use_checkpoint:  # and self.training:
            sens_maps = torch.utils.checkpoint.checkpoint(
                 self.sens_net, masked_kspace_tmean, mask, num_low_frequencies, mask_type, compute_sens_per_coil,
                use_reentrant=False)
        else:
            sens_maps = self.sens_net(masked_kspace_tmean, mask, num_low_frequencies, mask_type, compute_sens_per_coil)

        #if self.sens_net.use_sens_adj:
            #sens_maps = torch.chunk(sens_maps, self.num_adj_slices, dim=1)[self.center_slice]
        
        print(f"Shape of sens_maps: {sens_maps.shape}")
        print(f"Shape of masked_kspace: {masked_kspace.shape}")
