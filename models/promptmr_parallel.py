...
class ParallelPromptMR(PromptMR):
    ...
    def forward_parallel(
        ...
        # Reset state before pass B (optional but safer for distribution shift)
        history_feat = None
        latent = img_pred.clone()

        # Pass B: 0..N-1 (or a subset if chain_mode == "A_only")
        if hasattr(self, 'cascades_b'):
            for ith in range(self.num_cascades):
                run_b_this_index = (self.chain_mode == "AB") or (
                    self.chain_mode == "A_only" and ith in self._ab_index_set
                )
                if not run_b_this_index:
                    continue
                if use_checkpoint and self.training:
                    img_pred, latent, history_feat = torch.utils.checkpoint.checkpoint(
        ...
        if hasattr(self, '_debug_printed') and not self._debug_printed:
            print(f"🔍 串行架构执行了 {total_steps} 个cascade步骤")
