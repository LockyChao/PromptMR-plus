...
class UpBlock(nn.Module):
    def __init__(self, in_dim, out_dim, prompt_dim, n_cab, kernel_size, reduction, bias, act, no_use_ca=False, n_history=0):
        ...
        self.reduce = nn.Conv2d(
            in_dim+prompt_dim, in_dim, kernel_size=1, bias=bias)

        self.up = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                nn.Conv2d(in_dim, out_dim, 1, stride=1, padding=0, bias=False))

        self.ca = CAB(out_dim, kernel_size, reduction,
                      bias=bias, act=act, no_use_ca=no_use_ca)
...
class NormPromptUnet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, history_feat: Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]] = None, buffer: torch.Tensor = None):
        if not x.shape[-1] == 2:
...
class PromptMRBlock(nn.Module):
    ...
    def forward(
        ...
        ffx = self.sens_reduce(torch.where(
            mask, current_kspace, zero), sens_maps)
...
class PromptMR(nn.Module):
    ...
    def forward(
        ...
        img_zf = self.sens_reduce(masked_kspace, sens_maps)
        ...
        for cascade in self.cascades:
            if use_checkpoint:  # and self.training:
                img_pred, latent, history_feat = torch.utils.checkpoint.checkpoint(
                    cascade, img_pred, img_zf, latent, mask, sens_maps, history_feat,
                    use_reentrant=False)
            else:
