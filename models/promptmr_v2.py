...
class PromptMRBlock(nn.Module):
    ...
    def __init__(self, model: nn.Module, num_adj_slices=5):
        ...
        self.apply_dc = True  # 默认True，控制是否应用Data Consistency
        self.model_scale = 1.0  # 控制模型项强度，默认1.0
        self.use_buffer = True   # 控制是否启用adaptive input buffer
        self.use_history = True  # 控制是否启用历史特征
    ...
    def forward(
        ...
        if self.model.n_buffer > 0:
            ...
            if not self.use_buffer:
                buffer = buffer.detach() * 0.0
        else:
            buffer = None
            
        soft_dc = (ffx - img_zf) * self.dc_weight if self.apply_dc else 0.
        model_term, latent, history_feat = self.model(current_img, history_feat if self.use_history else None, buffer)
        if not self.use_history:
            history_feat = None
        img_pred = current_img - soft_dc - self.model_scale * model_term
