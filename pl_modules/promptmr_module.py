...
class PromptMrModule(MriModule):
    ...
    def __init__(
        ...
        parallel_mode: bool = False,  # 新增：是否使用并行cascade架构
        **kwargs,
    ):
        """
        Args:
            ...
            parallel_mode: whether to use parallel cascade architecture for inference
        """
        ...
        # 根据parallel_mode选择模型类
        if self.parallel_mode:
            try:
                ParallelPromptMR = get_model_class(f"models.{model_version}_parallel", "ParallelPromptMR")
                ...
                print("⚠️  ParallelPromptMR not found, falling back to standard PromptMR")
                PromptMR = get_model_class(f"models.{model_version}")
    ...
    def _load_pretrain_weights(self):
        ...
        if self.parallel_mode and hasattr(self.promptmr, 'copy_weights_from_original'):
            # 对于并行架构，需要特殊处理权重复制
