...
if __name__ == '__main__':
    ...
    if not args.symbol_only:
        for ff in tqdm(file_list):
            ...
            #meta file name is ff.split('/')[-1].split('.')[0] + '_info.csv'
            meta_file_name = ff.split('/')[-1].split('.')[0] + '_info.csv'
            meta_file_path = ff.replace(ff.split('/')[-1], meta_file_name)
            #read csv file from meta_file_name
            meta = pd.read_csv(meta_file_path)
            print(save_name, ftype, meta)

            # ---- gather meta information ----
            # 1) extract "Value" column as float array (may be empty)
            if 'Value' in meta.columns:
                # Convert to numeric; treat '' or non-numeric as NaN, then set to 0
                value_series = pd.to_numeric(meta['Value'].replace('', np.nan), errors='coerce')
                value_arr = np.nan_to_num(value_series.to_numpy(dtype=np.float32).ravel(), nan=0.0)
            else:
                value_arr = np.empty(0, dtype=np.float32)
            ...
            # ---- build numeric meta vector: concat Value array and ftype_int ----
            ftype_int = np.array([sum(ord(c) for c in ftype)], dtype=np.float32)
            meta_vec = np.concatenate([value_arr, ftype_int], axis=0)  # float32 array

            file.create_dataset('meta', data=meta_vec, dtype='float32')
